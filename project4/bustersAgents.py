...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    # Find most likely positions of ghosts:
    mostLikelyPositions = [i.argMax() for i in livingGhostPositionDistributions]

    # Find distances:
    finalPosition, closestDistance = None, 99999999
    for ghostPos in mostLikelyPositions:
    	dist = self.distancer.getDistance(ghostPos, pacmanPosition)
    	if dist < closestDistance:
    		closestDistance = dist
    		finalPosition = ghostPos

    # Find best action:
    bestAction, bestDistance = None, 99999999
    for action in legal:
    	successorPos = Actions.getSuccessor(pacmanPosition, action)
    	successorDist = self.distancer.getDistance(successorPos, finalPosition)
    	if successorDist < bestDistance:
    		bestAction = action
    		bestDistance = successorDist
    return bestAction