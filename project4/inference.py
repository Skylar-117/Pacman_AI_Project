  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
      ...
      for pos in legalPositions:
        realDist = util.manhattanDistance(pos, pacmanPosition)
        if emissionModel[realDist] > 0:
          allPossible[pos] = emissionModel[realDist] * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    self.particles = [legalPoses[random.randint(0, len(legalPoses)-1)] for i in range(numParticles)]
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
      ...
      for particle in self.particles:
        newDistance = util.manhattanDistance(particle, pacmanPosition)
        beliefs[particle] += emissionModel[newDistance]
    # Case (2):
      if not beliefs.totalCount():
        self.initializeUniformly(gameState)
      else:
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles):
      particles = [legalPoses[random.randint(0, len(legalPoses)-1)] for j in range(self.numGhosts)]
      self.particles.append(tuple(particles))
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      for i in range(self.numGhosts):
        ...
        gameState = setGhostPositions(gameState, prevGhostPositions)
        ghostIndex = i
        agnet =  self.ghostAgents[ghostIndex]
        newPosDist = getPositionDistributionForGhost(gameState, ghostIndex, agnet)
    ...
    for particle in self.particles:
      ...
      for i in range(self.numGhosts):
        if noisyDistances[i] != None:
          ...
          weight *= emissionModels[i][dist]
        else:
          particle[i] = self.getJailPosition(i)
      weights[tuple(particle)] += weight

    if not weights.totalCount():
      sumOfAllValues = sum(value[1] for value in weights.items())
      if not sumOfAllValues:
        self.initializeParticles()
        return

    self.particles = [util.sample(weights) for i in range(self.numParticles)]

  # def conversion(self, particle, index):
  #   particle = list(particle)
